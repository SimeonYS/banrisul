...
class BbanrisulPipeline:

    # Database setup
    conn = sqlite3.connect('banrisul.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `banrisul`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM banrisul WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO banrisul (date, title, link, content)"
